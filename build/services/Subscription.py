...
from config.config import ORION_HOST, ORION_PORT, FIWARE_SERVICE, FIWARE_SERVICEPATH, \
    SUBSCRIPTION_JSON_PATH, SUBSCRIPTION_JSON_FILENAME, SUBSCRIPTION_JSON_FILENAME_LD, SUBSCRIPTION_JSON_FILENAME_MQTT, SUBSCRIPTION_JSON_FILENAME_MQTT_LD, MULTIPLE_SUBSCRIPTIONS, ORION_SUBSCRIPTION
import requests
...
class Subscription:
    ...
    def splitAttrsForSubscription(self, jsonSubscription):
        ...
        if len(entities) > 1:
            logger.error(
                'Cannot create separate subscriptions with more than a single entity')
            return
        else:
            ...
            for attr in attrs:
                ...
                tempJsonSubscription["subject"]["condition"]["attrs"] = [
                    "{0}".format(attr)]
                tempJsonSubscription["notification"]["attrs"] = [
                    "{0}".format(attr)]
    ...
    def createOrionSubscription(self):
        if len(self.subscriptionsIds) > 0:
            logger.error(
                'Cannot create a new subscription, already exists: ' + str(self.subscriptionsIds))
            return
        validator = Validator()
        ...
        jsonSubscription = open(
            SUBSCRIPTION_JSON_PATH + '/' + SUBSCRIPTION_JSON_FILENAME, 'r').read()
        isValid = validator.validateOrionSubscriptionJSON(jsonSubscription)
        if isValid:
            if MULTIPLE_SUBSCRIPTIONS == "true":
                jsonSubscriptions = self.splitAttrsForSubscription(
                    json.loads(jsonSubscription))
                for jsonSub in jsonSubscriptions:
                    self.sendPOSTSubscriptionV2Request(
                        url, json.dumps(jsonSub), headers)
            else:
                self.sendPOSTSubscriptionV2Request(
                    url, jsonSubscription, headers)
        else:
        ...
        if isValid:
            if MULTIPLE_SUBSCRIPTIONS == "true":
                ...
                for index, jsonSub in enumerate(jsonSubscriptions):
                    self.sendPATCHSubscriptionV2Request(
                        url, json.dumps(jsonSub), headers, subscriptionId)
            else:
                self.sendPATCHSubscriptionV2Request(
                    url, jsonSubscription, headers, subscriptionId)
        else:
            logger.error('Invalid schema for ' + SUBSCRIPTION_JSON_FILENAME)
    ...
    def createOrionLDSubscription(self):
        if len(self.subscriptionsIds) > 0:
            ...
            return
        validator = Validator()
        url = 'http://{0}:{1}/ngsi-ld/v1/subscriptions'.format(
            ORION_HOST, ORION_PORT)
        headers = {'Accept': 'application/json',
            'Content-Type': 'application/ld+json'}
        jsonSubscription = open(
            SUBSCRIPTION_JSON_PATH + '/' + SUBSCRIPTION_JSON_FILENAME_LD, 'r').read()
        isValid = validator.validateOrionSubscriptionJSON(jsonSubscription)
        if isValid:
    ...
    def updateOrionLDSubscription(self):
        ...
        url = 'http://{0}:{1}/ngsi-ld/v1/subscriptions/{2}'.format(
            ORION_HOST, ORION_PORT, subscriptionId)
        headers = {'Accept': 'application/json'}
    ...
    # MQTT
    def createOrionMQTTSubscription(self):
        if len(self.subscriptionsIds) > 0:
            logger.error(
                'Cannot create a new subscription, already exists: ' + str(self.subscriptionsIds))
            return
        ...
        headers = {'Accept': 'application/json', 'Content-Type': 'application/json',
                'Fiware-Service': FIWARE_SERVICE, 'Fiware-ServicePath': FIWARE_SERVICEPATH}
        jsonSubscription = open(
            SUBSCRIPTION_JSON_PATH + '/' + SUBSCRIPTION_JSON_FILENAME_MQTT, 'r').read()
        isValid = validator.validateOrionSubscriptionJSON(jsonSubscription)
        if isValid:
            if MULTIPLE_SUBSCRIPTIONS == "true":
                jsonSubscriptions = self.splitAttrsForSubscription(
            ...
            logger.error('Invalid schema for ' +
                         SUBSCRIPTION_JSON_FILENAME_MQTT)

    # MQTT-LD
    def createOrionMQTTLDSubscription(self):
        if len(self.subscriptionsIds) > 0:
        ...
        jsonSubscription = open(SUBSCRIPTION_JSON_PATH + '/' + SUBSCRIPTION_JSON_FILENAME_MQTT_LD, 'r').read()
        isValid = validator.validateOrionSubscriptionJSON(jsonSubscription)
        if isValid:
            r = requests.post(url, data=jsonSubscription, headers=headers)
            if r.status_code == 201:
